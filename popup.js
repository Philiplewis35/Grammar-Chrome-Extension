...
function render_signed_in_page() {
  chrome.storage.local.get('gc_email', function(response) {
    ...
    $('body').html('<p>Hi ' + response["gc_email"] + '</p></br><a href="https://grammar-checker1.herokuapp.com/" target="_blank">Go to grammar checker</a></br><input type="submit" name="commit" value="Sync Services" class="gc_sync"></br><input type="submit" name="commit" value="Log out" class="gc_log_out">')
  });
}
