...
// log in
$(document).on('click', '.submit_log_in', function(){
  ...
  $.ajax({
    ...
    success: function(response) { // sign in
      render_signed_in_page()
      $('body').prepend('<p class="success">Signed In</p>')
    ...
    error: function() { // show error message on log in page
      if($('.error')[0]) {
        ...
        $('body').prepend('<p class="error">Invalid username or password</p>')
      }
    }
  });
})
...
$(document).on('click', '.log_out', function(){
  chrome.storage.local.get(['gc_email', 'gc_auth'], function(headers) {
    $.ajax({
      ...
      error: function() {
        if($('.error')[0]) {
          $('.error').html('Could not log out')
        } else {
          $('body').prepend('<p class="error">Could not log out</p>')
        }
      }
    })
  })
